...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const words = ["Full Stack Developer", "Backend Developer", "AI & Agentic AI Developer", "Software Engineer"];
    let wordIndex = 0;
    let charIndex = 0;
    ...
    if (hasFinePointer && cursorDot && cursorOutline) {
        ...
        const interactables = document.querySelectorAll('a, button, .floating-icon, .filter-btn, .project-btn, .profile-card, .cert-item');
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function (e) {
            ...
            if (message.length < 10) {
                if (formFeedback) {
                    formFeedback.textContent = 'Please write a message of at least 10 characters.';
                    formFeedback.className = 'form-feedback error';
                }
                return;
            }
    ...
    if (resumeModal && closeResumeBtn) {
        ...
        const closeModal = () => {
            ...
            if (openResumeBtn) openResumeBtn.focus();
        };
